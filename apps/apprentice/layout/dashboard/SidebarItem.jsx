...
function SubItem({ item }) {
  ...
  return (
    <Link
      href={item.href}
      className={cn(
        "flex items-center gap-2.5 rounded-lg py-1.5 pl-10.5 pr-3 text-[13px] font-medium transition-colors duration-100",
...
export function SidebarItem({ item }) {
  ...
  const baseRow = cn(
    "group relative flex w-full items-center gap-3 rounded-xl px-3 py-[9px] text-sm font-medium select-none transition-all duration-150 ease-out",
    highlighted
      ? "text-white"
  ...
  const activeRowStyle = highlighted
    ? {
        background:
          "linear-gradient(90deg,color-mix(in srgb,var(--portal-accent) 18%,transparent) 0%,color-mix(in srgb,var(--portal-accent) 6%,transparent) 100%)",
