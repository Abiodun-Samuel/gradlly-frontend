"use client";

import { GraduationCap, Pencil, Plus, Trash2 } from "lucide-react";
import { useState } from "react";

import Button from "@/components/ui/Button";
import { DataTable } from "@/components/ui/DataTable";
import { useRoleAccess } from "@/features/auth/hooks/useRoleAccess";
import { cn } from "@/utils/helper";

import { DeleteProgrammeModal } from "./DeleteProgrammeModal";
import { ProgrammeFormModal } from "./ProgrammeFormModal";
import { ProgrammeStatusBadge } from "./ProgrammeStatusBadge";
import { useProgrammes } from "../queries/programmes.query";

// ─── Cells ──────────────────────────────────────────────────────────────────

function ProgrammeCell({ programme }) {
  return (
    <div className="flex items-center gap-3">
      <div className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-primary-50 text-primary-600 ring-1 ring-primary-100">
        <GraduationCap className="size-4.5" strokeWidth={1.75} aria-hidden />
      </div>
      <div className="min-w-0">
        <p className="truncate font-medium text-neutral-900">
          {programme.title}
        </p>
        {programme.description ? (
          <p className="truncate text-xs text-neutral-400">
            {programme.description}
          </p>
        ) : null}
      </div>
    </div>
  );
}

function CodeCell({ code }) {
  return (
    <span className="inline-flex items-center rounded-md bg-neutral-100 px-2 py-0.5 font-mono text-xs font-medium text-neutral-600">
      {code}
    </span>
  );
}

function RowActions({ programme, onEdit, onDelete }) {
  return (
    <div className="flex items-center justify-end gap-1">
      <button
        type="button"
        onClick={() => onEdit(programme)}
        title="Edit programme"
        className={cn(
          "flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-medium",
          "text-neutral-600 hover:bg-neutral-100 hover:text-neutral-900",
          "transition-colors duration-150",
        )}
      >
        <Pencil className="size-3.5" aria-hidden />
        Edit
      </button>
      <button
        type="button"
        onClick={() => onDelete(programme)}
        title="Delete programme"
        className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-medium text-danger-600 transition-colors duration-150 hover:bg-danger-50 hover:text-danger-700"
      >
        <Trash2 className="size-3.5" aria-hidden />
        Delete
      </button>
    </div>
  );
}

// ─── Main component ───────────────────────────────────────────────────────────

export function ProgrammesTable() {
  // Owners and admins manage programmes; members get a read-only view.
  const { can } = useRoleAccess();
  const canManage = can("admin");

  const [page, setPage] = useState(1);
  const [perPage, setPerPage] = useState(20);
  const [formOpen, setFormOpen] = useState(false);
  const [editTarget, setEditTarget] = useState(null);
  const [deleteTarget, setDeleteTarget] = useState(null);

  const { data, isLoading, isFetching } = useProgrammes({ page, perPage });
  const programmes = data?.programmes ?? [];
  const meta = data?.meta ?? null;

  const handlePerPageChange = (next) => {
    setPerPage(next);
    setPage(1); // reset to the first page when page size changes
  };

  const openCreate = () => {
    setEditTarget(null);
    setFormOpen(true);
  };

  const openEdit = (programme) => {
    setEditTarget(programme);
    setFormOpen(true);
  };

  const closeForm = () => {
    setFormOpen(false);
    setEditTarget(null);
  };

  const columns = [
    {
      key: "title",
      header: "Programme",
      primary: true,
      sortable: true,
      sortValue: (row) => row.title,
      cell: (row) => <ProgrammeCell programme={row} />,
    },
    {
      key: "code",
      header: "Code",
      mobileLabel: "Code",
      sortable: true,
      sortValue: (row) => row.code,
      cell: (row) => <CodeCell code={row.code} />,
    },
    {
      key: "status",
      header: "Status",
      mobileLabel: "Status",
      sortable: true,
      sortValue: (row) => row.status,
      cell: (row) => <ProgrammeStatusBadge status={row.status} />,
    },
    ...(canManage
      ? [
          {
            key: "actions",
            header: "Actions",
            align: "right",
            mobileLabel: "Actions",
            cell: (row) => (
              <RowActions
                programme={row}
                onEdit={openEdit}
                onDelete={setDeleteTarget}
              />
            ),
          },
        ]
      : []),
  ];

  return (
    <div className="space-y-5">
      {/* ── Section header ── */}
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-base font-semibold text-neutral-900">
            All programmes
          </h2>
          <p className="mt-0.5 text-sm text-neutral-500">
            Top-level curriculum containers your standards belong to.
          </p>
        </div>
        {canManage ? (
          <Button
            size="sm"
            color="green"
            startIcon={<Plus className="size-4" />}
            onClick={openCreate}
          >
            New programme
          </Button>
        ) : null}
      </div>

      <DataTable
        columns={columns}
        data={programmes}
        rowKey={(row) => row.id}
        isLoading={isLoading}
        meta={meta}
        onPageChange={setPage}
        onPerPageChange={handlePerPageChange}
        empty={{
          icon: GraduationCap,
          title: "No programmes yet",
          description: canManage
            ? "Create your first programme to start building standards and enrolments."
            : "There are no programmes at this time.",
          action: canManage ? (
            <Button
              size="sm"
              color="green"
              startIcon={<Plus className="size-4" />}
              onClick={openCreate}
            >
              New programme
            </Button>
          ) : null,
        }}
        className={cn(
          isFetching && !isLoading && "opacity-70 transition-opacity",
        )}
      />

      <ProgrammeFormModal
        open={formOpen}
        onClose={closeForm}
        programme={editTarget}
      />
      <DeleteProgrammeModal
        programme={deleteTarget}
        open={Boolean(deleteTarget)}
        onClose={() => setDeleteTarget(null)}
      />
    </div>
  );
}
